...
\factoidbox{
A sentence can be symbolized as $\enot\meta{\phi}$ if it can be paraphrased in English as `It is not the case that \meta{\phi}'.
}
...
There are other words besides `although' and `but' that can be used to express conjunction.  For example, `Barbara is energetic, however she is not athletic' and `Barbara is energetic despite not being athletic' expresses the same conjunctive claim as sentence \eref{and6} from above, and get symbolized using the same TFL sentence. In general, the symbolization guideline for conjunction is:
	...
	\factoidbox{
		A sentence can be symbolized as $(\meta{\phi}\eand\meta{\psi})$ if (nuance aside) it can be paraphrased in English as `Both \meta{\phi} and \meta{\psi}'.
}
...
Sentence \eref{or2} is only slightly more complicated.  Here `or' occurs between two names rather than two complete sentences. However, we can paraphrase sentence \eref{or2} as `Either Denison will play golf with me, or Ellery will play golf with me' where `or' now connects two complete sentences. So we can symbolize it as `$(D \eor E)$'.
	\factoidbox{
		A sentence can be symbolized as $(\meta{\phi}\eor\meta{\psi})$ if it can be paraphrased in English as `Either \meta{\phi} or \meta{\psi}.'
	}
...
\begin{itemize}
\item[] $\enot(\meta{\phi} \eor \meta{\psi})$ is equivalent to $(\enot \meta{\phi} \eand \enot \meta{\psi})$
\item[] $\enot(\meta{\phi} \eand \meta{\psi})$ is equivalent to $(\enot \meta{\phi} \eor \enot \meta{\psi})$
\end{itemize}}
...
Since `if and only if' gets used so much in logic and philosophy, it is often abbreviated with a single, snappy word, `iff'. I shall follow this practice. So `if' with only \emph{one} `f' is the English conditional. But `iff' with \emph{two} `f's is the English biconditional. Armed with this we can say:
	\factoidbox{
		A sentence can be symbolized as $(\meta{\phi} \eiff \meta{\psi})$ if it can be paraphrased in English as `\meta{\phi} if and only if \meta{\psi}', that is, `\meta{\phi} iff \meta{\psi}'.
	}
...
So we can use the following guideline:
	\factoidbox{
		If a sentence can be paraphrased as `Unless \meta{\phi}, \meta{\psi}' or `\meta{\psi} unless \meta{\phi}', then it can be symbolized as $(\enot\meta{\phi} \eif \meta{\psi})$, or simply $(\meta{\phi}\eor\meta{\psi})$.
	}
...
Instead, we will describe the process by which sentences can be \emph{constructed}. Consider negation: given any sentence \meta{\phi} of TFL, putting a negation in front of it gives us a sentence $\enot\meta{\phi}$ .
We can say similar things for each of the other connectives. For instance, if \meta{\phi} and \meta{\psi} are sentences of TFL, then $(\meta{\phi}\eand\meta{\psi})$ is a sentence of TFL.  (What's up with the funny Greek letters, which are \emph{not} in the lexicon of TFL?  We'll get to that in \S\ref{s:TFLMetavariables} below.)
...
Providing clauses like this for all of the connectives, we arrive at the following \define{Syntax}, or formal definition of what counts as a  \define{sentence of TFL}:
	...
	\begin{enumerate}
		...
		\item If \meta{\phi} is a sentence, then $\enot\meta{\phi}$ is a sentence.
		\item If \meta{\phi} and \meta{\psi} are sentences, then $(\meta{\phi}\eand\meta{\psi})$ is a sentence.
		\item If \meta{\phi} and \meta{\psi} are sentences, then $(\meta{\phi}\eor\meta{\psi})$ is a sentence.
		\item If \meta{\phi} and \meta{\psi} are sentences, then $(\meta{\phi}\eif\meta{\psi})$ is a sentence.
		\item If \meta{\phi} and \meta{\psi} are sentences, then $(\meta{\phi}\eiff\meta{\psi})$ is a sentence.
...
For this reason, variables like `$\meta{\phi}$' and `$\meta{\psi}$' are called \define{Metavariables}: they form part of our metalanguage, English, and they range over arbitrary expressions in our object language, TFL.  But again, these metavariables are \emph{only} part of our metalanguage, and not themselves included in the object language TFL.


	\factoidbox{
		Greek letters $\meta{\phi}$, \meta{\psi}, \meta{\chi} etc. are \define{metavariables} in our \define{metalanguage}, used to talk about arbitrary expressions of TFL.  Roman letters $A, B, C \ldots$ etc. are atomic sentences of our \define{object language} TFL .}

So what clause 3 in our definition says is that if $\meta{\phi}$ and $\meta{\psi}$ are any arbitrary sentences of TFL, then the result of writing whatever $\meta{\phi}$ is, followed by `$\eand$', followed by whatever \meta{\psi} is, and enclosing the result in parentheses, produces a sentence of TFL.  For example, if \meta{\phi} is the TFL sentence `$\enot D$' and \meta{\psi} is the TFL sentence `$(S \rightarrow T)$', then clause 3 tells us that `$(\enot D \eand (S \rightarrow T))$' is a sentence of TFL.  But again, the string of symbols `$(\meta{\phi} \eand \meta{\psi})$' is not itself a sentence (nor even an expression) of TFL.
