...
The way scope works with quantifiers is very similar to it works with negation, which we discussed in \S\ref{s:TFLSyntacticNotions}.   In the sentence `$\enot P \eand \enot Q$', the first `$\enot$' has scope over just the first conjunct, and the second `$\enot$' has scope over just the second conjunct.   It is the conjunction symbol `$\eand$' that is the main logical operator.  Similarly, in `$\exists xLx \eand \exists xAx$', the first quantifier has scope over just the first conjunct, and the second quantifier over just the second conjunct, with the conjunction `$\eand$; functioning as the main logical operator.
...
Students who are first learning FOL symbolization sometimes want to put all the quantifiers out front, at the beginning of the sentence, perhaps like this:
...
If we let `$F$' be a one-place predicate,`$G$' a three-place predicate, and `$S$' a six-place predicate, then the following all count as atomic formulas of FOL by our definition:
	\begin{center}
		...
		$Sx_1 x_2 a b y x_6$\\
	\end{center}
Given the notion of an \emph{atomic formula}, we can now recursively define the broader class of \define{FOL formulas} as follows:
	\factoidbox{
	\begin{enumerate}
		...
		\item If \meta{\varphi} is a formula and $\meta{v}$ is a variable, then $\forall\meta{v}\meta{\varphi}$ is a formula.
		\item If \meta{\varphi} is a formula and $\meta{v}$ is a variable, then $\exists \meta{v}\meta{\varphi}$ is a formula.
...
Given the above syntactic rules, we can now define what the main operator of an FOL formula is and, with it, the notion of the scope of an operator:
	\factoidbox{
...
Since quantifiers are a kind of logical operator, this definition covers the scope of quantifiers alongside the scope of truth functional connectives.  The scope of the various quantifiers in the example formula constructed above can be illustrated as follows:
...
\noindent You may have noticed that our syntactic rules let us put a quantifier in front of \emph{any} formula, which lets us construct peculiar formulas like `$\forall x Fc$' or `$\forall x\exists xFx$'.  We'll discuss these below, after we've introduced some more terminology.
...
\subsection{Sentences of FOL}\label{s:FOLSentences}
With our definition of formulas in place, we're now ready to define the notion of a \emph{sentence} of FOL.  To see why we need to distinguish sentences from mere formulas of FOL, recall that logic is concerned with \emph{statements}: sentences  that can be either true or false. And many formulas are not true or false.  For example, consider the formulas $Fc$ and $Fx$, and suppose we have the following symbolization key:
	\begin{ekey}
		\item[\text{domain}] people
...
Since we want all sentences of FOL to be either true or false, we need to exclude formulas like `$Fx$' from the class of sentences.  We can do this by giving a precise definition of the notions of \define{bound} and \define{free} variables that we already informally discussed in \S \ref{s:Quantifiers}:

	\factoidbox{
	An occurrence of a variable \meta{v} is \define{bound} iff it falls within the scope of a matching quantifier $\forall \meta{v}$ or $\exists \meta{v}$.	
	
	A \define{free} variable is a variable that is not bound.
...
The universal quantifier `$\forall x$' has scope over the antecedent `$\forall x (Ex \eor Dy)$', so the `$x$' in `$Ex$' is bound, but the `$y$' in `$Dy$' is free. The scope of the existential quantifier `$\exists z$' is the consequent `$\exists z(Ex \eif Lzx)$', so `$z$' is bound.  But the two occurrences of `$x$' in the consequent are free: since they occur outside the scope of the universal quantifier `$\forall x$', they aren't in the scope of any matching quantifier.  

We can now single out the \define{sentences} of FOL from among the formulas as follows:

	\factoidbox{
	A \define{sentence} of FOL is a formula that contains no free variables.  Sentences are also called \define{closed formulas}.
	
	An \define{Open Formula} of FOL is a formula that contains at least one free variable.

		
	}

Our example `$(\forall x(Ex \eor Dy) \eif \exists z(Ex \eif Lzx))$' from above is thus an \emph{open formula} since it contains free occurrences of the variables `$y$' and  `$x$'.  We could transform it into a \emph{closed formula}, containing no free variables, by giving `$\forall x$' scope over the entire formula, thereby binding every occurrence of `$x$', and adding a quantifier to bind `$y$', as in for example:
$$\forall x((Ex \eor \exists yDy) \eif \exists z(Ex \eif Lzx))$$
This is now a \emph{sentence} of FOL.   By requiring that every variable in a sentence be bound, we ensure that all sentences of FOL are capable of being true or false.

As noted earlier, our syntactic rules also let us construct strange formulas like `$\forall x Fc$' and `$\forall x\exists xFx$'.   These are unusual because the formulas the quantifier `$\forall x$' attaches to are in both cases already closed formulas: neither `$Fc$' nor `$\exists x Fx$' contains any free variables.  These are instances of what is called \define{vacuous quantification}, cases where a quantifier (`$\forall x$'  in this case) binds no variables.  In general we can say:
\factoidbox{A quantifier $\forall \meta{v}$ or $\exists \meta{v}$ \define{binds} every occurrence of the matching variable $\meta{v}$ that (i) is in its scope, and (ii) is not within  the scope of any ``closer'' quantifier $\forall \meta{v}$ or $\exists \meta{v}$ itself occurring in the scope of the original quantifier.

%In a formula or subformula of the form $\forall \meta{v}\varphi$ (or $\exists \meta{v} \varphi$), the quantifier $\forall \meta{v}$ (or $\exists \meta{v}$)  \define{binds} every occurrence of the  variable $\meta{v}$ that is free in $\varphi$.

}
To illustrate: in the sentence `$\forall x(Fx \eif \exists xGx)$', the universal quantifier `$\forall x$' only binds the `$x$' in `$Fx$', since that is the only occurrence of `$x$' within its scope that isn't within the scope of a closer quantifier. It does not bind the variable in `$Gx$', because that variable occurs within the scope of the closer existential quantifier `$\exists x$'.  It is ``closer'' in the sense that `$\exists x$' itself occurs within the scope of the universal `$\forall x$'. `$\exists x$' is therefore what binds the variable in `$Gx$'.  And again, in a cases of vacuous quantification like  `$\forall x Fc$' and `$\forall x\exists xFx$', the quantifier `$\forall x$' binds no variables.  These sentences are equivalent to plain `$Fc$' and `$\exists xFx$', with the vacuous quantifier `$\forall x$' deleted.\footnote{We could perhaps say that in `$\forall x Fc$' the quantifier binds the `$x$' that accompanies the quantifier itself, since it occurs within its scope.  But I prefer to think of occurrences of variables, in the sense at issue in our definitions, as occurrences in the argument positions of predicates.  So `$\forall x Fc$' contains no occurrences of `$x$', in the relevant sense, for the quantifier to bind, and `$\forall x\exists xFx$' contains only one occurrence, bound by `$\exists x$'.}
