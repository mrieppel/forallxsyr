...
\factoidbox{
$\enot \meta{\phi}$ is true in a given interpretation \textbf{iff} $\meta{\phi}$ false in that interpretation\\

$\meta{\phi} \eand \meta{\psi}$ is true in a given interpretation \textbf{iff} both $\meta{\phi}$ and $\meta{\psi}$ are true in that interpretation\\

$\meta{\phi} \eor \meta{\psi}$ is true in a given interpretation \textbf{iff} either $\meta{\phi}$ or $\meta{\psi}$ is true in that interpretation\\


$\meta{\phi} \eif \meta{\psi}$ is true in a given interpretation \textbf{iff} either $\meta{\phi}$ is false or $\meta{\psi}$ is true in that interpretation\\

$\meta{\phi} \eiff \meta{\psi}$ is true in a given interpretation \textbf{iff} $\meta{\phi}$ has the same truth value as $\meta{\psi}$ in that interpretation
	}
...
\factoidbox{$\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true in a given interpretation \script{I} \textbf{iff}  $\meta{\phi}(\ldots \meta{v}^o \ldots)$ is true on \script{I} for \emph{every object} $o$ in \script{I}'s domain.\\

$\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true in a given interpretation \script{I} \textbf{iff}  $\meta{\phi}(\ldots \meta{v}^o \ldots)$ is true on \script{I} for \emph{at least one} object $o$ in \script{I}'s domain.
}
The idea, again, is that we go through each object $o$ in the domain, and check whether $\meta{\phi}(\ldots \meta{v} \ldots)$ is true when \meta{v} is treated as referring to $o$; if so, the universal sentence $\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true, if not, it is false.  Notice that it's much easier for an existential sentence to be true: for an existential $\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$, we only have to find \emph{one} object in the domain that satisfies $\meta{\phi}(\ldots \meta{v} \ldots)$.


This in turn means that that it's very easy for a universal sentence $\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ to be \emph{false}: we just have to find a single object relative to which $\meta{\phi}(\ldots \meta{v} \ldots)$ is false.  For an existential sentence $\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ to be false, on the other hand, we have to make sure that $\meta{\phi}(\ldots \meta{v} \ldots)$ is false for \emph{every} object in the domain. Let's state these ``falsity conditions'' for quantified sentences explicitly too:
\factoidbox{$\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is false in a given interpretation \script{I} \textbf{iff}  $\meta{\phi}(\ldots \meta{v}^o \ldots)$ is false on \script{I} for \emph{at least one object} $o$ in \script{I}'s domain.\\

$\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is false in a given interpretation \script{I} \textbf{iff}  $\meta{\phi}(\ldots \meta{v}^o \ldots)$ is false on \script{I} for \emph{every} object $o$ in \script{I}'s domain.
}
...
Defining truth in FOL was a bit tricky, due to the presence of quantifiers. But now that we know what determines the truth value of an FOL sentence in an interpretation, we can use that to define various other central logical notions.   As you can see, these definitions are basically the same as those from TFL, except that we here use the notion of truth in an \emph{interpretation} rather than truth on a \emph{valuation}.  In all the definitions below, the metavariables $\meta{\phi}_1 \ldots \meta{\phi}_n$ and $\meta{\psi}$ range over arbitrary \emph{sentences} of FOL:\footnote{Our logical concepts are in other words only defined for FOL sentences.  By contrast, the official explanation of truth in FOL, given in the Appendix, covers formulas in general, not just sentences.}

\factoidbox{
	$\meta{\phi}_1, \ldots, \meta{\phi}_n$ \define{logically entail}  $\meta{\psi}$, written $\meta{\phi}_1, \ldots, \meta{\phi}_n \vDash \meta{\psi}$,   iff no interpretation makes all of $\meta{\phi}_1,  \ldots, \meta{\phi}_n$ true but $\meta{\psi}$ false.\\

\meta{\phi} is a \define{Logical Truth}, written $\vDash \meta{\phi}$, iff it is true in every interpretation.\\

\meta{\phi} is a \define{contradiction}  iff it is false in every interpretation.\\

\meta{\phi} and \meta{\psi} are \define{logically equivalent}, written $\meta{\phi} \lequiv \meta{\psi}$ iff they have the same truth value in every interpretation.\\

$\meta{\phi}_1,\ldots, \meta{\phi}_n$ are \define{jointly consistent} iff there is at least one interpretation which makes them all true.  They are \define{jointly inconsistent} iff there is no such interpretation.
}
...
$$\meta{\phi}_1, \ldots, \meta{\phi}_n \therefore \meta{\psi}$$
is \define{valid} just in case its premises $\meta{\phi}_1, \ldots, \meta{\phi}_n$ entail its conclusion \meta{\psi}.  Since entailment is defined in terms of the concept of truth-in-an-interpretation, we can use interpretations to investigate the validity of FOL arguments.
...
Take the concept of logical truth first.  To show that some FOL sentence \meta{\phi} is \emph{not} a logical truth, it suffices to construct an interpretation that makes it false.  And to show that it \emph{is} a logical truth, we can give a natural deduction that proves \meta{\phi} as a theorem.  This gives us a way to approach contradictions as well: since \meta{\phi} is a contradiction iff $\enot\meta{\phi}$ is a logical truth, we can show that \meta{\phi} is a contradiction by proving $\enot\meta{\phi}$ as a theorem.  And to show that \meta{\phi} is \emph{not} a contradiction, it suffices to construct an interpretation in which it is true.


Next consider logical equivalence.  To show that \meta{\phi} and \meta{\psi} are \emph{not} logically equivalent, all we need to do is construct an interpretation on which one of them is true and the other is false.  And to show that \meta{\phi} and \meta{\psi} \emph{are} equivalent, we can give a natural deduction that proves $\meta{\phi} \eiff \meta{\psi}$ as a theorem, since \meta{\phi} and \meta{\psi} are equivalent just in case $\meta{\phi} \eiff \meta{\psi}$ is a logical truth.
...
\factoidbox{$$\text{If } \meta{\phi}_1, \ldots, \meta{\phi}_n \entails \bot \text{, then } \meta{\phi}_1, \ldots, \meta{\phi}_n \text{ are jointly inconsistent} $$
}
For suppose  $\meta{\phi}_1, \ldots, \meta{\phi}_n \entails \bot$.  This means there's no interpretation that makes all of $\meta{\phi}_1, \ldots, \meta{\phi}_n $ true but $\bot$ false.  However, since every interpretation makes $\bot$ false, this just means that there's no interpretation that makes all of $\meta{\phi}_1, \ldots, \meta{\phi}_n $ true.  That is, $\meta{\phi}_1, \ldots, \meta{\phi}_n $ are inconsistent.    So if we want to show that $\meta{\phi}_1, \ldots, \meta{\phi}_n $ are inconsistent, we can do that by giving a natural deduction proof showing that $\meta{\phi}_1, \ldots \meta{\phi}_n \proves \bot $.
...
\factoidbox{
$\enot \meta{\phi}$ is true on [] in \meta{I}  \textbf{iff} $\meta{\phi}$ false on [] in \meta{I} \\

$\meta{\phi} \eand \meta{\psi}$ is true on [] in \meta{I} \textbf{iff} both $\meta{\phi}$ and $\meta{\psi}$ are true on [] in \meta{I} \\

$\meta{\phi} \eor \meta{\psi}$ is true on [] in \meta{I}  \textbf{iff} either $\meta{\phi}$ or $\meta{\psi}$ are true on [] in \meta{I} \\

$\meta{\phi} \eif \meta{\psi}$ is true on [] in \meta{I} \textbf{iff} either $\meta{\phi}$ is false or $\meta{\psi}$ is true on [] in \meta{I}\\

$\meta{\phi} \eiff \meta{\psi}$ is true on [] in \meta{I} \textbf{iff} $\meta{\phi}$ and $\meta{\psi}$ have the same truth value on [] in \meta{I}\\

$\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true on [] in \meta{I}  \textbf{iff} for \emph{every object} $o$ in the domain, $\meta{\phi}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in \meta{I}\\

$\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true on [] in \meta{I}  \textbf{iff} for \emph{at least one object} $o$ in the domain, $\meta{\phi}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in \meta{I}.
}
In the claues governing the quantifiers, [\meta{v}:o] is the assignment that's just like our arbitrary assignment [] in every respect, except that it is stipulated to assign the object $o$ to the variable \meta{v}.  The idea, again, being that we go through each object $o$ in the domain, and check whether the formula $\meta{\phi}(\ldots \meta{v} \ldots)$ is true when $o$ is assigned to the variable \meta{v}.  If so, $\forall \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ is true, if not, it is false.  On the other hand, for an existential sentence $\exists \meta{v}\meta{\phi}(\ldots \meta{v} \ldots)$ to be true, it's enough if \emph{at least one} object makes $\meta{\phi}(\ldots \meta{v} \ldots)$ true.
...
\factoidbox{For any FOL sentence \meta{\phi} and any interpretation \meta{I}, \meta{\phi} is true in \meta{I} iff \meta{\phi} is true in \meta{I} on any assignment [] whatsoever.
}
