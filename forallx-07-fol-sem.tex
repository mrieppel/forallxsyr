...
\chapter{Semantics of FOL}\label{ch:FOLSemantics}
...
As we discussed in the last section, to directly specify the extension of a \emph{many-place} predicate, we have to give a list of \emph{tuples} of objects, rather than individual objects, to indicate in what order the predicate holds of the objects.  So for example, if we take the sentence `$\forall xRxx$' which contains the two-place predicate `$R$', one possible interpretation would be the following:
	\begin{ekey}
...
Similarly, `$Rab$' is true on our interpretation just in case the referent of `$a$' is less than the referent of `$b$'.  Since 2 is indeed less than 3, `$Rab$' is true.  By contrast, `$Rba$' is not true, because 3 is not less than 2, or to put it another way, the pair \ntuple{3,2} is not in the extension of `$R$' on our interpretation.  Similarly, neither `$Raa$' nor `$Rbb$' are true on our go-to interpretation, since neither 2 nor 3 is less than itself (i.e. the pairs \ntuple{2,2} and \ntuple{3,3} are not in the extension of `$R$').  In general we can say:
	\factoidbox{
		When \meta{R} is an $n$-place predicate and $\meta{a}_1, \ldots, \meta{a}_{n}$ are names, the sentence $\meta{R}\meta{a}_{1} \ldots\meta{a}_{n}$ is true in a given interpretation \textbf{iff} $\meta{R}$ is true of the objects referred to by $\meta{a}_{1}, \ldots, \meta{a}_{n}$ (in that order) in that interpretation	}
...
\paragraph{Adding Truth Functional Connectives} Just like in TFL, sentences of FOL can be built up from simpler ones using the truth-functional operators. The semantic rules governing these operators are exactly the same as they were when we considered TFL. Here they are:
	...
	\factoidbox{
$\meta{A} \eand \meta{B}$ is true in a given interpretation \textbf{iff} both $\meta{A}$ and $\meta{B}$ are true in that interpretation\\		
		
$\meta{A} \eor \meta{B}$ is true in a given interpretation \textbf{iff} either $\meta{A}$ or $\meta{B}$ is true in that interpretation\\

$\enot \meta{A}$ is true in a given interpretation \textbf{iff} $\meta{A}$ false in that interpretation\\

$\meta{A} \eif \meta{B}$ is true in a given interpretation \textbf{iff} either $\meta{A}$ is false or $\meta{B}$ is true in that interpretation\\

$\meta{A} \eiff \meta{B}$ is true in a given interpretation \textbf{iff} $\meta{A}$ has the same truth value as $\meta{B}$ in that interpretation
	}
...
The big innovation in FOL is the use of \emph{quantifiers}. And specifying the truth conditions for quantified sentences turns out to be a little tricky.  If we only look at simple sentences, things are pretty straightforward.  We can say that `$\exists xEx$' is true iff `$E$' is true of at least one object in the domain, and  `$\forall xEx$' is true iff `$E$' is true of every object in the domain.  So on our go-to interpretation, `$\exists xEx$' comes out true, and `$\forall xEx$' comes out false.
...
But the number 4 does \emph{not} satisfy it: `$(Ex \rightarrow Rxb)$' is \emph{false} on the assignment $[x:4]$.  That's because `$Ex$' is true on $[x:4]$ (since 4 is even) but `$Rxb$' is false on $[x:4]$ (since 4 is \emph{not} less than 3).  So  since `$(Ex \rightarrow Rxb)$' is false for some object in the domain (namely 4), we can conclude that the universal sentence `$\forall x(Ex \rightarrow Rxb)$' we started with is itself false.  And that's of course the result we want: relative to our go-to interpretation, `$\forall x(Ex \rightarrow Rxb)$' says that every even number is less than three, which is clearly false, since there are lots of even numbers, including 4, that are not less than 3.

In general, then, we explain the truth conditions for quantified sentences as follows:
\factoidbox{$\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true on [] in a given interpretation  \textbf{iff} for \emph{every object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in that interpretation.\\

$\exists \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true on [] in a given interpretation  \textbf{iff} for \emph{at least one object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in that interpretation.
}
Here [] is just an arbitrary assignment, and  [\meta{v}:o] is an assignment that treats the variable \meta{v} as referring to o, and treats all other variables just like [] does (whatever that might be).  The idea, in other words, is that we go through each object $o$ in the domain, and check whether $\meta{A}(\ldots \meta{v} \ldots)$ is true when \meta{v} is treated as referring to $o$; if so, the universal sentence $\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true, if not, it is false.  Notice that it's much easier for an existential sentence to be true: for an existential $\exists \meta{v}\meta{A}(\ldots \meta{v} \ldots)$, we only have to find \emph{one} object in the domain that satisfies $\meta{A}(\ldots \meta{v} \ldots)$.


This in turn means that that it's very easy for a universal sentence $\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ to be \emph{false}: we just have to find a single object relative to which $\meta{A}(\ldots \meta{v} \ldots)$ is false.  For an existential sentence $\exists \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ to be false, on the other hand, we have to make sure that $\meta{A}(\ldots \meta{v} \ldots)$ is false for \emph{every} object in the domain. Let's state these ``falsity conditions'' for quantified sentences explicitly too:
\factoidbox{$\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is false on [] in a given interpretation  \textbf{iff} for \emph{at least one object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is false on [\meta{v}:o] in that interpretation.\\

$\exists \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is false on [] in a given interpretation  \textbf{iff} for  \emph{every object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is false on [\meta{v}:o] in that interpretation.
}
...
\item $(Fx^2 \eand Gx^2)$ is false (since $Fx^2$ is false), and
\item $(Fx^3 \eand Gx^3)$ is false (since $Fx^3$ is false)
\end{etriangle}
\end{etriangle}
...
\noindent Notice that the main operator in this is $\eand$, i.e. it's a conjunction.  So to determine its truth value, we have to evaluate its two conjuncts $\exists xFx$ and $\exists xGx$ \emph{separately}.  Both of them turn out to be true, and so the conjunction is true:
\begin{etriangle}
...
Defining truth in FOL was a bit tricky, due to the presence of quantifiers. But now that we know what determines the truth value of an FOL sentence in an interpretation, we can use that to define various other central logical notions.   As you can see, these definitions are basically the same as those from TFL, except that we here use the notion of truth in an \emph{interpretation} rather than truth on a \emph{valuation}.  In all the definitions below, the metavariables $\meta{A}_1 \ldots \meta{A}_n$ and $\meta{C}$ range over arbitrary \emph{sentences} of FOL:\footnote{Our logical concepts are in other words only defined for FOL sentences.  By contrast, the official explanation of truth in FOL, given in the Appendix, covers formulas in general, not just sentences.}  

\factoidbox{
	$\meta{A}_1, \ldots, \meta{A}_n$ \define{logically entail}  $\meta{C}$, written $\meta{A}_1, \ldots, \meta{A}_n \vDash \meta{C}$,   iff no interpretation makes all of $\meta{A}_1,  \ldots, \meta{A}_n$ true but $\meta{C}$ false.\\

\meta{A} is a \define{Logical Truth}, written $\vDash \meta{A}$, iff it is true in every interpretation.\\

\meta{A} is a \define{contradiction}  iff it is false in every interpretation.\\

\meta{A} and \meta{B} are \define{logically equivalent}, written $\meta{A} \lequiv \meta{B}$ iff they have the same truth value in every interpretation.\\

$\meta{A}_1,\ldots, \meta{A}_n$ are \define{jointly consistent} iff there is at least one interpretation which makes them all true.  They are \define{jointly inconsistent} iff there is no such interpretation.
}
...
is \define{valid} just in case its premises $\meta{A}_1, \ldots, \meta{A}_n$ entail its conclusion \meta{C}.  Since entailment is defined in terms of the concept of truth-in-an-interpretation, we can use interpretations to investigate the validity of FOL arguments.
...
And luckily this also makes the first premise `$\forall x(Rx \eif Mx)$' true!  After all, $(Rx^1 \eif Mx^1)$ is true (since $Rx^1$ is false), and since 1 is the only object in our domain, that suffices for `$\forall x(Rx \eif Mx)$'  to be true.
...
1   &   $+$  & $-$  \\ \hline
\end{tabular}\end{center}
The trouble is that this interpretation will also make our premise `$\exists x(Fx \eif Gx)$' false.  After all, $(Fx^1 \eif Gx^1)$ is false, and there's no other object in the domain we could assign to `$x$' to make `$(Fx \eif Gx)$' true. 
...
What we can conclude is just that the original English argument is not \emph{formally} valid, or more specifically, that it isn't valid in virtue of the kind of logical form captured in its FOL symbolization.  But it might still be valid for other reasons --- in this case, because of the connection between the meanings of `fox' and `vixen'.  As we discussed in \S\ref{s:FormalValidity}, logic doesn't aim to capture the validity of arguments like this; it only cares about formally valid arguments.\footnote{Of course we can make the above argument formally valid by adding a second premise: `Every vixen is a fox', which we'd symbolize as `$\forall x(Vx \eif Fx)$'.  But this is now a \emph{different} argument, one with two premises.}
...
\begin{ekey}
	...
	\item[R] \blank \ is less than \blank
\end{ekey}
We can't specify this interpreation ``directly'' by listing all the objects in the domain and all the pairs in the extension of `$R$', since there are too many of them.  But we can specify the extension of `$R$' indirectly, via an English predicate. This interpretation does what we want:
\begin{ebullet}
\item `$\forall x\exists yRxy$' is true, since for every positive integer $x$ we can find a $y$ that $x$ is less than..
\item `$\forall x \forall y \forall z((Rxy \eand Ryz) \eif Rxz)$' is true, since if $x$ is less and $y$ and $y$ is less than $z$, it follows that $x$ must be less than $z$.
\item `$\exists xRxx$' is false, since there exist no positive integer that is less than itself.
\end{ebullet}
...
Take the concept of logical truth first.  To show that some FOL sentence \meta{A} is \emph{not} a logical truth, it suffices to construct an interpretation that makes it false.  And to show that it \emph{is} a logical truth, we can give a natural deduction that proves \meta{A} as a theorem.  This gives us a way to approach contradictions as well: since \meta{A} is a contradiction iff $\enot\meta{A}$ is a logical truth, we can show that \meta{A} is a contradiction by proving $\enot\meta{A}$ as a theorem.  And to show that \meta{A} is \emph{not} a contradiction, it suffices to construct an interpretation in which it is true.


Next consider logical equivalence.  To show that \meta{A} and \meta{B} are \emph{not} logically equivalent, all we need to do is construct an interpretation on which one of them is true and the other is false.  And to show that \meta{A} and \meta{B} \emph{are} equivalent, we can give a natural deduction that proves $\meta{A} \eiff \meta{B}$ as a theorem, since \meta{A} and \meta{B} are equivalent just in case $\meta{A} \eiff \meta{B}$ is a logical truth.
...
\factoidbox{$$\text{If } \meta{A}_1, \ldots, \meta{A}_n \entails \bot \text{, then } \meta{A}_1, \ldots, \meta{A}_n \text{ are jointly inconsistent} $$
}
For suppose  $\meta{A}_1, \ldots, \meta{A}_n \entails \bot$.  This means there's no interpretation that makes all of $\meta{A}_1, \ldots, \meta{A}_n $ true but $\bot$ false.  However, since every interpretation makes $\bot$ false, this just means that there's no interpretation that makes all of $\meta{A}_1, \ldots, \meta{A}_n $ true.  That is, $\meta{A}_1, \ldots, \meta{A}_n $ are inconsistent.    So if we want to show that $\meta{A}_1, \ldots, \meta{A}_n $ are inconsistent, we can do that by giving a natural deduction proof showing that $\meta{A}_1, \ldots \meta{A}_n \proves \bot $.
...
\factoidbox{
		When \meta{R} is an $n$-place predicate and $\meta{t}_1, \ldots, \meta{t}_{n}$ are terms, the formula $\meta{R}\meta{t}_{1} \ldots\meta{t}_{n}$ is true on [] in \meta{I} \textbf{iff} $\meta{R}$ is true of the objects referred to by $\meta{t}_{1},  \ldots, \meta{t}_{n}$ (in that order) on [] in \meta{I}.	\\
...
\factoidbox{
$\enot \meta{A}$ is true on [] in \meta{I}  \textbf{iff} $\meta{A}$ false on [] in \meta{I} \\

$\meta{A} \eand \meta{B}$ is true on [] in \meta{I} \textbf{iff} both $\meta{A}$ and $\meta{B}$ are true on [] in \meta{I} \\		
		
$\meta{A} \eor \meta{B}$ is true on [] in \meta{I}  \textbf{iff} either $\meta{A}$ or $\meta{B}$ are true on [] in \meta{I} \\

$\meta{A} \eif \meta{B}$ is true on [] in \meta{I} \textbf{iff} either $\meta{A}$ is false or $\meta{B}$ is true on [] in \meta{I}\\

$\meta{A} \eiff \meta{B}$ is true on [] in \meta{I} \textbf{iff} $\meta{A}$ and $\meta{B}$ have the same truth value on [] in \meta{I}\\

$\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true on [] in \meta{I}  \textbf{iff} for \emph{every object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in \meta{I}\\

$\exists \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true on [] in \meta{I}  \textbf{iff} for \emph{at least one object} $o$ in the domain, $\meta{A}(\ldots \meta{v} \ldots)$ is true on [\meta{v}:o] in \meta{I}.
}
Again, in the quantifier clauses [\meta{v}:o] is the assignment that's just like our arbitrary assignment [] in every respect, except that it is stipulated to assign the object $o$ to the variable \meta{v}.  The idea, again, being that we go through each object $o$ in the domain, and check whether the formula $\meta{A}(\ldots \meta{v} \ldots)$ is true when $o$ is assigned to the free variable \meta{v}.  If so, $\forall \meta{v}\meta{A}(\ldots \meta{v} \ldots)$ is true, if not, it is false.

Given all of this, we can now say what is required for any \emph{sentence}, i.e. any formula with no free variables, to be true in an interpretation \meta{I}:

\factoidbox{For any FOL sentence \meta{A} and any interpretation \meta{I}, \meta{A} is true in \meta{I} iff \meta{A} is true in \meta{I} on any assignment whatsoever.
}
With all these pieces in place, the definition of the various logical concepts (like entailment) now proceeds as it did in \S\ref{s:FOLSemanticConcepts}.
